...
const EditToolTab = (route: any) => {
  const paramData = route?.params?.tool;
    ...
    onSubmit: async (values: FormValuesType) => {
      ...
      try {
        let response = await fetch(
          `${appConfig.baseUrl}/projects/${selectedProject.id}/rooms/${values.room}/tools/${toolId}/`,
          {
            method: 'PUT',
            headers: {
  ...
  return (
    <View style={{flex: 1}}>
      ...
      <ViewContainer style={{...styles.container, zIndex: 3}}>
        <ViewContainer flex={1} height={80}>
          <DropDown
            ...
            label={'Assignee'}
            placeholderStyle={paramData.assignee ? {color: Colors.black} : {}}
            placeholder={
              paramData.assignee
      ...
      <ViewContainer style={{...styles.container, zIndex: 2}}>
        <ViewContainer flex={1} height={80}>
          <DropDown
            placeholderStyle={paramData.room ? {color: Colors.black} : {}}
            placeholder={
              paramData.room
      ...
      <View>
        ...
        <View style={styles.buttonContainer}>
          <CustomButton
            title={localization.room.updateRoom}
            disabled={
              !formik.isValid
